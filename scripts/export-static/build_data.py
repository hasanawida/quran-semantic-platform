...
#
# 30MB ← 64MB مع متون المعاجم الأربعة، ثم ← 96MB مع تفاسير خمسة
# (قرار المالك 2026-08-01). المقيس بعدها ≈ 89MB: التفسير 45 والمعاجم
# 22.8 والصرف 10.6 والباقي متفرق.
#
...
def main() -> None:
    ...
    #
    # لماذا: فحصُ المعاجم (docs/audits/LEXICON_SOURCING.md)
    ...
    # مسدَّ المعنى العربي.
    #
    # **والبديل العربي قيد البناء (2026-07-30):** «مختار الصحاح»، طبعة
    # المطبعة الأميرية ١٩٢٠م — ملكية عامة نصًّا وترقيمًا. تُنسخ صفحاتها
    # نسخًا بصريًّا وتُراجَع بشريًّا صفحةً صفحة (§24.6)، ولا يُنشر إلا
    # المراجَع. التفصيل: data/transcriptions/mukhtar-sihah-1920/README.md
    sihah_path = DATA / "sihah_bundle.json.gz"
    sihah_meta: dict | None = None
    sihah_roots: list[str] = []
    if sihah_path.exists():
        with gzip.open(sihah_path, "rt", encoding="utf-8") as handle:
            sihah = json.load(handle)
        sihah_meta = sihah["meta"]
        shards: dict[str, dict] = {}
        for root_key, entry in sihah["entries"].items():
            assert root_key in roots, f"مادة لجذر ليس عندنا: {root_key}"
            shards.setdefault(root_key[0], {})[root_key] = entry
        for letter, payload in shards.items():
            write(f"lexicon/sihah/{ord(letter):04x}.json", payload)
    ...
    if tafsir_path.exists():
        ...
        # ---- للاستزادة: إحالاتٌ لا متون ----
        # كتبٌ نافعة لا يجوز نقلُ متنها: مؤلفوها معاصرون والنصّ من
        # إنشائهم هم — فلا يُفصَل «متنٌ قديم» عن «جهاز حديث» كما في
        # كتب القدماء (§٢٠). فتُسمّى بإسنادها ويُدلّ القارئ على موضعها،
        # وطلبُ الإذن مسجَّل في docs/permissions/REQUESTS.md — فمتى وصل
        # جوابٌ مكتوب دخل المتن كما دخل غيره.
        tafsir_meta = dict(tafsir_meta or {})
        tafsir_meta["further_reading"] = [
            {
                "work": "صفوة التفاسير",
                "author": "محمد علي الصابوني",
                "died": "١٤٤٢هـ / ٢٠٢١م",
                "edition": "دار الصابوني — القاهرة",
                "why": (
                    "تفسيرٌ ميسَّر مختصر، جمع فيه مؤلفه زبدةَ الطبري "
                    "وابن كثير والقرطبي والكشاف بأسلوبٍ قريب — نافعٌ "
                    "لمن أراد المعنى مجموعًا في إيجاز"
                ),
                "status": "غير مُدخَل — حقُّ المؤلف قائم، والإذن مطلوب",
                "url": "https://search.worldcat.org/search?q=صفوة+التفاسير+الصابوني",
            },
            {
                "work": "تيسير الكريم الرحمن في تفسير كلام المنان",
                "author": "عبد الرحمن بن ناصر السعدي",
                "died": "١٣٧٦هـ / ١٩٥٧م",
                "edition": "تحقيق عبد الرحمن اللويحق — مؤسسة الرسالة",
                "why": "أوجزُ التفاسير المعاصرة وأيسرُها عبارةً",
                "status": "غير مُدخَل — حقُّ المؤلف قائم، والإذن مطلوب",
                "url": "https://search.worldcat.org/search?q=تيسير+الكريم+الرحمن+السعدي",
            },
            {
                "work": "أضواء البيان في إيضاح القرآن بالقرآن",
                "author": "محمد الأمين الشنقيطي",
                "died": "١٣٩٣هـ / ١٩٧٣م",
                "edition": "دار الفكر — بيروت",
                "why": "أصلٌ في تفسير القرآن بالقرآن",
                "status": "غير مُدخَل — حقُّ المؤلف قائم، والإذن مطلوب",
                "url": "https://search.worldcat.org/search?q=أضواء+البيان+الشنقيطي",
            },
        ]
        tafsir_meta["further_reading_note"] = (
            "الاستزادةُ متنٌ يُقرأ هنا لا رابطٌ يُتبع: فوق تفسيرَي البغوي "
            "وابن كثير، أُدخلت ثلاثة أصولٍ أخرى كاملةً — الجلالين "
            "لإيجاز المعنى، والمحرَّر الوجيز لابن عطية للتحليل اللغوي، "
            "والكشاف للزمخشري للبلاغة. وهذه الكتبُ الثلاثة الأخيرة "
            "بعينها ممّا اختصره الصابوني في «صفوة التفاسير» بنصّ "
            "مقدمته. أمّا ما دونها فمؤلفوه معاصرون ونصُّهم من إنشائهم، "
            "فلا يُنقل منه حرفٌ بلا إذنٍ مكتوب — يُسمَّى بإسناده ليُقرأ "
            "عند صاحبه، وطلبُ الإذن مسجَّل في docs/permissions/REQUESTS.md؛ "
            "ومتى وصل جوابٌ دخل المتن كما دخل غيره."
        )
    ...
    write(
        "lexicon.json",
        {
            ...
            "reason": (
                ...
                "الطبعة + الملف + الترخيص + البصمة» — فاختارت المنصة طريق "
                "الطبعات الحرّة: يُنسخ «مختار الصحاح» (المطبعة الأميرية "
                "١٩٢٠م، ملكية عامة) نسخًا بصريًّا صفحةً صفحة، ولا تُنشر "
                "مادةٌ قبل مراجعةٍ بشرية لصفحتها (§24.6). فما تراه بمتنٍ "
                "فمراجَع، وما تراه «غير مُدخَل» فلم يبلغه النسخ أو لم "
                "تبلغه المراجعة بعد. ولا يُسدّ الفراغ بمتنٍ مجهول الطبعة "
                "ولا بترجمةٍ آلية."
            ),
            "audit": "docs/audits/LEXICON_SOURCING.md",
            # ---- الإحالة بدل النقل ----
            # نُسمّي أفضل طبعةٍ لكل معجم بمحقّقها وناشرها، ونعطي القارئ
            # الطريق إليها — **ولا نَنقل متنًا**. والنسبة إقرارٌ بالمصدر لا
            # إذنٌ منه، فلا يُغني ذكرُ الطبعة عن استئذان صاحبها.
            # والترتيب بوفاة المؤلف: زمنيٌّ لا تفضيليّ (ADR-013).
            "references": [
                {
                    "work": "كتاب العين",
                    "author": "الخليل بن أحمد الفراهيدي",
                    "died": "١٧٠هـ",
                    "edition": "تحقيق مهدي المخزومي وإبراهيم السامرائي — دار ومكتبة الهلال",
                    "why": "أقدم معجم عربي، ومرتَّبٌ بالجذر على المخارج",
                    "url": "https://shamela.ws/book/1682",
                },
                {
                    "work": "تهذيب اللغة",
                    "author": "أبو منصور الأزهري",
                    "died": "٣٧٠هـ",
                    "edition": "تحقيق محمد عوض مرعب — دار إحياء التراث العربي، ط١ ٢٠٠١م",
                    "why": "يجمع اللغة بأسانيدها ويناقش ما في العين",
                    "url": "https://shamela.ws/book/7031",
                },
                {
                    "work": "الصحاح — تاج اللغة",
                    "author": "إسماعيل بن حماد الجوهري",
                    "died": "٣٩٣هـ",
                    "edition": "دار العلم للملايين — بيروت، ط٤ ١٤٠٧هـ/١٩٨٧م",
                    "why": "يقتصر على الصحيح الفصيح",
                    "url": "https://shamela.ws/book/23235",
                },
                {
                    "work": "معجم مقاييس اللغة",
                    "author": "أحمد بن فارس القزويني",
                    "died": "٣٩٥هـ",
                    "edition": "تحقيق عبد السلام محمد هارون — البابي الحلبي، ط٢",
                    "why": (
                        "أنفعُها لهذه المنصة: يردّ كل جذرٍ إلى أصلٍ معنويّ "
                        "واحد أو أصولٍ معدودة — وهو عين الاستقراء الدلالي"
                    ),
                    "url": "https://shamela.ws/book/21710",
                },
                {
                    "work": "المفردات في غريب القرآن",
                    "author": "الراغب الأصفهاني",
                    "died": "٥٠٢هـ",
                    "edition": "تحقيق صفوان عدنان الداودي — دار القلم والدار الشامية، ط١ ١٤١٢هـ",
                    "why": "مقصورٌ على ألفاظ القرآن، فيلتحم بفهرس الجذور مباشرة",
                    "url": "https://shamela.ws/book/23636",
                },
                {
                    "work": "لسان العرب",
                    "author": "ابن منظور الإفريقي",
                    "died": "٧١١هـ",
                    "edition": "دار صادر — بيروت، ط٣ ١٤١٤هـ",
                    "why": "أجمعُها للمادة اللغوية",
                    "url": "https://shamela.ws/book/1687",
                },
                {
                    "work": "عمدة الحفاظ في تفسير أشرف الألفاظ",
                    "author": "السمين الحلبي",
                    "died": "٧٥٦هـ",
                    "edition": "تحقيق محمد باسل عيون السود — دار الكتب العلمية، ط١ ١٤١٧هـ/١٩٩٦م",
                    "why": "معجمٌ لألفاظ القرآن مع وجوه الإعراب",
                    "url": "https://shamela.ws/book/17829",
                },
                {
                    "work": "تاج العروس من جواهر القاموس",
                    "author": "محمّد مرتضى الحسيني الزَّبيدي",
                    "died": "١٢٠٥هـ",
                    "edition": "تحقيق جماعة من المختصين — (الطبعة غير متحقَّقة عندنا)",
                    "why": "أوسعُ المعاجم العربية مادةً",
                    "url": "https://shamela.ws/book/7030",
                },
            ],
            "references_note": (
                "هذه إحالاتٌ لا متون. تُسمّى الطبعة ومحقّقها وناشرها ليعرف "
                "القارئ أين يقرأ، ويُقرأ النصّ عند صاحبه لا عندنا. وذكرُ "
                "المصدر إقرارٌ به لا إذنٌ منه — وطلباتُ الإذن مسجَّلة في "
                "docs/permissions/REQUESTS.md، فمتى وصل جوابٌ مكتوب دخل المتن."
            ),
            # سجلّ المصادر بحقيقة حالها — مقروءًا من مواقع أصحابها
            "sources": [
                {
                    "name": (
                        "مختار الصحاح للرازي — المطبعة الأميرية ١٩٢٠م · "
                        "مصوَّرة مكتبة الإسكندرية"
                    ),
                    "url": "https://archive.org/details/AAlexandrina-196404",
                    "licence": (
                        "ملكية عامة — نصًّا وترقيمًا معًا (طُبع قبل ١٩٢٩)، "
                        "فلا حقَّ لأحدٍ فيه ولا إذنَ يُطلب."
                    ),
                    "status": "transcribing",
                    "note": (
                        "الطريق المعتمد: نسخٌ بصريّ حديث من صور المصوَّرة "
                        "— بعد أن قِيست المحرّكات الحرّة فأسقطت الضبط "
                        "(٧٪) وهو في المعجم المحتوى نفسه. تُنسخ الصفحات "
                        "تباعًا ولا تُنشر مادةٌ قبل مراجعةٍ بشرية لصفحتها "
                        "(§24.6). وما يُنتَج يُعاد نشره حرًّا."
                    ),
                },
                {
